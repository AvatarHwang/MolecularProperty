...
def run(rank, size):
    ...
    args.cuda=True
    args.data_path='exampledata/finetune/tg403.csv'#'exampledata/finetune/tox21.csv'
    args.dataset_type='multiclass'#'classification'
    args.dense=False
    args.depth=6
    args.dist_coff=0.1
    ...
    args.features_path=['exampledata/finetune/tg403.npz']#['exampledata/finetune/tox21.npz']
    ...
    args.save_dir='model/finetune/tg403/'#'model/finetune/tox21/'
    ...
    #args.task_names=['vapour', 'aerosol', 'gas']#['NR-AR', 'NR-AR-LBD', 'NR-AhR', 'NR-Aromatase', 'NR-ER', 'NR-ER-LBD', 'NR-PPAR-gamma', 'SR-ARE', 'SR-ATAD5', 'SR-HSE', 'SR-MMP', 'SR-p53']
    args.tensorboard=False
    args.test_fold_index=None
    #args.train_data_size=6264
    args.undirected=False
    args.use_compound_names=False
    args.use_input_features=['exampledata/finetune/tg403.npz']#['exampledata/finetune/tox21.npz']
...
def run_training(args, logger, group_node, group_edge, tag_id):
    ...
    num_workers=int(torch.multiprocessing.cpu_count() / (world_size/2))
    ...
    if rank%2 == 0:
        ...
        if world_size>2:
            train_sampler=torch.utils.data.distributed.DistributedSampler(train_data, num_replicas=int(world_size/2), shuffle=False, hybrid=True)
